...
/// Add an extra strike market intraday for a stock.
/// Same as create_market but callable on the same date as existing markets.
/// Admin-only. Allows dynamic strike creation during trading hours.
pub fn handler(
    ...
    require!(
        ticker.len() >= 1 && ticker.len() <= 8,
        MeridianError::InvalidTicker
    );
    require!(
        strike_price > 0 && strike_price % 1000 == 0,
    ...
    market.vault_bump = 0;
    market.escrow_yes_bump = 0;
    market.bid_escrow_bump = 0;
...
/// AddStrike uses the same PDA scheme as CreateMarket.
/// The `init` constraint ensures no duplicate market exists for this ticker/strike/date.
#[derive(Accounts)]
